...
function validation($request){  //$_POST連想配列
    ...
    if(empty($request['your_name']) || 20 < mb_strlen($request['your_name']) ){
      ...
      if(empty($request['email']) || !filter_var($request['email'], FILTER_VALIDATE_EMAIL)){ //emailの形式ではなかったらの意味で「!」を入れている
        $errors[] = '「メールアドレス」は必須です。正しい形式で入力してください。';
      }

      if(!empty($request['url'])){  //空ではなかったら判定をかけて、URLではなかったらこのエラーを表示する
